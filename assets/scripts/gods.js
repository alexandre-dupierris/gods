...
if (sauvegarde) {
  ...
  mesMobs = genererMobs()
  physique = physique
  monInventaire = Array(20)
    .fill()
    .map(() => ({ id: null, quantite: 0 }))
  tempsJeu = 0
  joueur = joueur
...
document.addEventListener('click', function (e) {
  ...
  for (const position of positionsMoutons) {
    const distanceSquared =
      Math.pow(position.x + 20 - clickX, 2) +
      Math.pow(position.y + 20 - clickY, 2)
    if (distanceSquared < 1200 && !travaux && !objetPorte.id) {
      // 40px de rayon au carré
...
document
  ...
  .addEventListener('click', sauvegarderPartie)
